    ...
    {foreach $order_details as $detail}
        <tr data-order-detail-id="{$detail.id_order_detail}">
            <td class="text-center">
                ...
                {assign var="isCurrentManifest" value=($manifest_id == $detail.id_manifest)}
                {assign var="shouldDisable" value=($hasManifest && !$isCurrentManifest)}

                <input type="checkbox" data-manifestid="{$manifest_id}" class="order-detail-checkbox"
                    name="selected_order_details[]" value="{$detail.id_order_detail}" data-order-id="{$detail.id_order}"
                    data-product-name="{$detail.product_name|escape:'html':'UTF-8'}" {if $isCurrentManifest} checked="checked"
                        {/if} {if $shouldDisable} disabled="disabled" {/if} />
