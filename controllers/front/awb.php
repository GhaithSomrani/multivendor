...
/**
 * Pickup Manifest Generation Controller
 */
...
class MultivendorAwbModuleFrontController extends ModuleFrontController
{
    public $auth = true;
    public $ssl = true;
    public $vendor;
    ...
    public function initContent()
    {
        ...
        // Verify this order detail belongs to the vendor
        if (!$this->verifyOrderDetailOwnership($id_order_detail)) {
            die('Access denied');
        }

        // Generate PDF
        $this->generatePickupPdf($id_order_detail);
    }
     ...
     */
    protected function generatePickupPdf($id_order_detail)
    {
        ...
        $pdf->SetTitle('Pickup Manifest - ' . $order->reference);
        ...
        $pdf->Output('Pickup_Manifest_' . $order->reference . '_' . $id_order_detail . '.pdf', 'D');
        exit;
    }
}